...
- **Resolved during the final whole-feature review (was: "known gap carried over from the spec, not resolved by this plan"):** whether any backend action (void, discount, close bill) is gated to `assigned_waiter_id` specifically rather than `Role.WAITER` + floor access. A targeted grep of every `assigned_waiter_id` reference outside `table-operators.service.ts`/`orders.service.ts`, plus a full read of `permissions.service.ts`, confirms **no such gate exists anywhere in the backend** — authorization is Role + floor access (`user_floor_access`) + flag overrides + "does a bill already exist," never operator identity. The design spec's "full permission parity for a joined-but-not-primary steward" promise is genuinely delivered, not accidentally broken.
...
## Known gap found during Task 10 code review (not fixed, tracked for a follow-up)

`_serverTableToLocal`'s `occupiedSince` logic (`lib/services/sync_service.dart`) computes a per-device, per-operator approximation of "how long has this table been occupied": it reads `existing?.occupiedSince` from the CURRENT device's own local `tablesProvider` state, falling back to a per-table SharedPreferences timer cache, falling back to `DateTime.now()`. This was written under the old single-owner model, where a table could only ever go `free → mine` (never `other → mine` without passing through `free`) on a given device. This feature breaks that assumption: a steward joining an already-occupied table now transitions directly from `other` to `mine` on their own device, with no local timer-cache entry and no `existing?.occupiedSince` to inherit — so `DateTime.now()` is used, and the joining steward's UI will show "just occupied" even if the table has genuinely been in service for 45+ minutes.

This is a cosmetic/UX gap, not a data-integrity issue — no crash, no data loss, and the original steward's own device is unaffected (confirmed by code review). It exists because neither the design spec nor this plan surfaces a per-table or per-operator "occupied since" timestamp from the backend (the `operators` array only carries `{operator_id, operator_name}`, not `joined_at`, even though `table_operators.joined_at` already exists server-side and is used internally for primary-waiter derivation). Fixing this properly would mean: (a) adding `joined_at` (or the table's own first-occupied time) to the `operators`/table payload sent to clients, and (b) using the earliest such timestamp as the authoritative `occupiedSince` instead of reconstructing it per-device. Out of scope for this 11-task plan — flagged here as a candidate follow-up task before/alongside any UI work that makes table-occupied-duration visible to a newly-joining steward.

## Feature complete — final whole-diff review findings (all Minor, non-blocking)

The final review across both repos' full commit ranges (backend `eb25f6e..HEAD`, client `b20b3b5..HEAD`) found the socket contract, ack/broadcast shapes, and every documented fix/regression-test all check out end-to-end, with 69/69 backend tests and 15/15 client tests passing. Two Minor items surfaced, neither blocking:

- `table_operators.operator_id REFERENCES users(id)` has no `ON DELETE CASCADE`/`SET NULL` (unlike `table_id`, which does cascade). Hard-deleting a `users` row while that user is mid-table-join would leave an orphaned, invisible `table_operators` row (silently dropped by the `INNER JOIN users` in `getOperatorsForTable`, but never cleaned up). Low real-world likelihood; worth a follow-up if user hard-deletion is ever exercised in practice.
- `_pillLabel` on the tables grid shows only the earliest joiner's name as the compact badge — a reasonable space-constrained UI choice, not a bug, but worth knowing it's a different (single-name) presentation than the order builder header's full list of joined stewards.
